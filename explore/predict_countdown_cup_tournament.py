...
from predict_functions import build_rmsa_map, calculate_tournament_table, predict_match, \
    predict_tournament_cycle, sort_season_table, predict_hawaii
...
for i in range(0, 10000):
    tournament_lp = predict_hawaii(Teams.Hunters, Teams.Dynasty, Teams.Reign, Teams.Gladiators, rmsa_map, week_three_rotation)
    season_east, season_west = calculate_tournament_table(season_results)
...
all_east_results.to_csv('../results/cc_final_sim_results_east.csv', index=False)
all_west_results.to_csv('../results/cc_final_sim_results_west.csv', index=False)
